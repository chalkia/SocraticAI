...
export function renderTeacherScreen(container, lang) {
    // --- ΦΑΣΗ 1: SETUP SCREEN (GRID LAYOUT) ---
    container.innerHTML = `
            ...
            <div class="teacher-grid">
                ...
                <div class="sidebar-col">
                    <div class="card auth-card sticky-card">
                        <h3>🚀 2. Launch Class</h3>
                        
                        <div class="settings-box">
                            <label>Max Questions / Group:</label>
                            <input type="number" id="max-messages" value="15" style="font-size:1.2em; padding:5px;">
                        </div>

                        <div class="settings-box">
                            <input type="checkbox" id="research-consent-check">
                            <label for="research-consent-check" style="display:inline;">${getTranslation(lang, 'research_consent')}</label>
                        </div>
                        ...
                        <div class="api-box">
                            ...
                            <input type="password" id="personal-api-key" placeholder="Paste Gemini Key">
                            <button id="save-personal-key-btn" class="secondary-btn small-btn">Save Key Locally</button>
                            <p id="key-status-text" class="status-text">No Key Loaded</p>
                        </div>

                        <button id="start-session-btn" class="primary-btn big-start-btn">
                            START CLASS
                        </button>
                    </div>
                </div>
    ...
    document.getElementById('save-personal-key-btn').addEventListener('click', () => {
        const key = document.getElementById('personal-api-key').value.trim();
        if(key) {
            localStorage.setItem('gemini_api_key', key);
            const status = document.getElementById('key-status-text');
    ...
    document.getElementById('start-session-btn').addEventListener('click', async () => {
        ...
        if (!apiKey) return alert("Please load an API Key.");

        const context = document.getElementById('setup-context').value || "General Tutor";
        const grade = document.getElementById('setup-grade').value || "General";
        const goal = document.getElementById('setup-goal').value || "Learn";
        const method = document.getElementById('setup-method').value || "Socratic";
        const rules = document.getElementById('setup-rules').value || "No limits";
